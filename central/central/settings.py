...
env_debug = os.environ.get("DEBUG", None)    # This will mainly change the behaviour from debug or not, debug = true should be local dev only since it will use SQLite
env_load_balancer = os.environ.get("LOAD_BALANCER", None)   # Need to know if deployed behind a balancer so proxy settings can be configured
...
REST_FRAMEWORK = {    
    ...
    #With value of 1 always get the latest IP in the X-Forwarded-For header as that's the one added by the load balancer.
    #When not behind a load balancer, this value should be 0
