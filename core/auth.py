...
from django.contrib.auth.password_validation import validate_password, password_changed, \
    get_default_password_validators, password_validators_help_texts
...
PASSWORD_RECOVERY_EMAIL_TEXT_HTML = \
    u"""<html><body>
    <p>Please use the below token on the password recovery form. <span style='color:red'>
    If you did not request this, ignore this email.</span></p><br/>
    <b>{0}</b>
    </body></html>
    """
...
def authenticate_user(email, password, validate_additional=True, update_last_login=False,
                      ex_class=AuthenticationFailed):
    """
...
def validate_user_jwt(token, validate_additional=True, ex_class=AuthenticationFailed):
    ...
    try:
        payload = jwt.decode(token, SECRET_KEY, True, JWT_DECODE_AGLS,
                             options=JWT_OPTIONS)  # Handles exp and signature validation.
...
def create_user_password_recovery_token(user):
    ...
    payload = {
        'id': user.pk,
        'last': user.last_password_change.isoformat() if user.last_password_change else "",
        # Use last password changed date so token is invalidated on change.
